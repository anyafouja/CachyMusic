...
class MusicPlayer:
    ...
    async def create_embed_data(self, vc: wavelink.Player):
        ...
        # Simplified layout as requested
                ...
                try:
                    ...
                    if self.np:
                        try:
                            ...
                            pass
                    self.np = await self._channel.send(embed=embed, file=file)
                except Exception:
                    pass
                ...
                try:
                    ...
                    while (vc.playing or vc.paused) and self._guild.voice_client:
                        ...
                        if count % 5 == 0 and self.np:
                            try:
                                embed, file = await self.create_embed_data(vc)
                                await self.np.edit(embed=embed, attachments=[file])
                            except Exception:
                                pass
    ...
    async def pause_(self, ctx):
        ...
        if vc.playing:
            ...
            if player.np:
                ...
                await player.np.edit(embed=embed, attachments=[file])
